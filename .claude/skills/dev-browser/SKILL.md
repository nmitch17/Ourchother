...
// Option 1: Explicit login
if (hasLoginCredentials()) {
  const result = await login(page);
  console.log(result.success ? "Logged in!" : result.message);
}

// Option 2: Auto-login when redirected
await page.goto("http://localhost:3000/dashboard");
await waitForPageLoad(page);

const loginResult = await autoLoginIfNeeded(page, "http://localhost:3000/dashboard");
if (loginResult) {
...
```bash
# Project A (.env.local)
DEV_BROWSER_LOGIN_EMAIL=admin@projecta.com
DEV_BROWSER_LOGIN_PASSWORD=projecta-password

# Project B (.env.local)
DEV_BROWSER_LOGIN_EMAIL=dev@projectb.com
DEV_BROWSER_LOGIN_PASSWORD=projectb-password
```
